...
parser.add_argument('--input_annotation_txt_dir', type=str, help='输入存储图片的标注txt文件(注意不要有中文)')
parser.add_argument('--output_anchors_txt',  type=str, help='输出的存储Anchor boxes的文本文件')
parser.add_argument('--input_num_anchors',  default=6, type=int, help='输入要计算的聚类（Anchor boxes的个数）')
parser.add_argument('--input_cfg_width', type=int, help="配置文件中width")
parser.add_argument('--input_cfg_height',  type=int, help="配置文件中height")
...
if __name__ == '__main__':
    ...
    args.input_annotation_txt_dir = 'data/labels'
    args.output_anchors_txt = '123456.txt '
    args.input_num_anchors = 6
    args.input_cfg_width = 640
    args.input_cfg_height = 480
    ...
    # 将anchor写入文件并保存
    f_anchors = open(args.output_anchors_txt, 'a')
    #
    for anchor in anchors:
        finishstr='%d,%d' % (int(anchor[0] * args.input_cfg_width), int(anchor[1] * args.input_cfg_height))
        f_anchors.write(finishstr)
        f_anchors.write('\n')
        print(finishstr)